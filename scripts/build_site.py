...
REPORTS = [
    ...
    {"dst":"deep-ep","entry":"DeepEP_Final_Analysis_Report.html","visual":"DeepEP",
     "title":"DeepEP 深度设计分析 (Survey by AI) (审核中)",
...
# ──── 生成首页 Hero Aside (Use This Page) — 动态最新文章 + 正确计数 ────
def gen_hero_aside(posts):
    # 最新手写文章
    latest_post = posts[0] if posts else None
    # 最新 survey 报告 (按 REPORTS 数组顺序,第一个为最新)
    ...
    lp_html = ""
    if latest_post:
        lp_html = (f'<li><span>Latest post</span>'
                   f'<a href="{latest_post["url"]}">{latest_post["title"]}</a>'
                   f' — {latest_post["excerpt"][:120]}…</li>')

    ls_html = ""
    if latest_report:
        ls_html = (f'<li><span>Latest survey</span>'
                   f'<a href="{latest_report["dst"]}/{latest_report["entry"]}">{latest_report["title"]}</a>'
                   f' — {latest_report["desc"][:120]}…</li>')

    browse_html = (f'<li><span>Browse all</span>'
                   f'<a href="posts.html">{posts_count} posts</a> on GPU, memory, CXL, MTP, and infrastructure economics · '
                   f'<a href="#survey">{reports_count} AI survey projects</a> with paper-level depth.</li>')

    return (f'<aside class="hero-aside" aria-label="How to use this site">\n'
            f'    <p class="aside-kicker">Use This Page</p>\n'
            f'    <h2 class="aside-title">Two kinds of content, one through-line: first-principles thinking.</h2>\n'
            f'    <p class="aside-text">Posts are hand-written deep dives. Survey reports are AI-generated with full source tracing. Both aim to make tradeoffs explicit and claims quantifiable.</p>\n'
            f'    <ul class="aside-list">\n'
            f'      {lp_html}\n'
            f'      {ls_html}\n'
            f'      {browse_html}\n'
            f'    </ul>\n'
...
.article-body h4{{font-size:1.1rem;font-weight:600;margin:1.2em 0 .4em;padding-left:8px;border-left:2px solid var(--muted);color:var(--fg2)}}
...
def main():
    ...
    # Hero Aside: 动态生成最新文章链接 + 正确计数
    # 匹配 <aside class="hero-aside"...</aside> 整体替换 (不依赖注释占位符)
    hero_aside_full = gen_hero_aside(posts)
    idx = re.sub(r'<aside class="hero-aside".*?</aside>',
                 hero_aside_full, idx, flags=re.S)
