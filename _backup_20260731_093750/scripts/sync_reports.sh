...
# ------------------------------------------------------------------------------
PROJECTS=(
  "umdk_research|umdk|analysis/cam_v2/CAM深度分析报告_v2.html|🔬|UMDK/CAM 深度分析 v2|CAM 通信加速库 8 章深度分析 v2 (2026-07-27) · EP Normal Zero-Buffer 流水线 / EP Low-Latency 8+4 阶段细粒度同步 / Fused Deep MoE 三变体 (BF16/W4A8/FWK) + Catlass GEMM / Detour All-to-All+ReduceScatter / A2E-E2A 跨域通信 / 框架基础设施 (CamComm/Socket/Pybind) / 双层CMake+HFA+OPX / 跨章节设计洞察 (Detour/MC2/Magic+Value)|chip|p0"
  ...
  "inference-community-2026|inference-community|web/sources/coreweave_inference.html|🌐|推理社区 2026|全球推理社区前沿动态 · CoreWeave Particula LSYS 等 · 开源与商业化|inference|p1"
  "DeepEP_research|deep-ep|DeepEP_Final_Analysis_Report.html|⚙️|DeepEP 深度设计分析|DeepSeek 开源 DeepEP 库三视角深度设计分析 · MoE 专家并行 AllToAll 通信 / NVLink+RDMA 双域融合 / Low-Latency 内核 / Normal 内核 / 性能基准与竞品对比|network|p0"
  "deepgemm_research/docs|deepepv2|html/index.html|🔬|DeepGEMM & DeepEP Survey by AI|25 篇三向对比分析报告（架构4篇 + 博客↔DeepGEMM 10篇 + 三向对比10篇）· 博客理论 ⇌ DeepEP源码 ⇌ DeepGEMM源码 · 核心发现：同步范式 Barrier→mbarrier FIFO / 通信模型 消息传递→Load-Store 对称内存直传|chip|p0"
)
# ==============================================================================
